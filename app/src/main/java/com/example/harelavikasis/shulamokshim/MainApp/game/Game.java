...
import com.example.harelavikasis.shulamokshim.MainApp.board.Board;
import com.example.harelavikasis.shulamokshim.MainApp.board.BoardSquare;
import com.example.harelavikasis.shulamokshim.MainApp.exceptions.InitializationException;
import com.example.harelavikasis.shulamokshim.MainApp.board.TileView;
...
public class Game {
    ...
    public void stopTimer() {
        if(mStartTime > 0) {
            ...
            lastElapsedTime = (int)(mElapsedTime/1000);
            Log.d("uniNote",  "mElapsedTime: " + lastElapsedTime);
            // Reset timer
            mStartTime = 0;
        }
    }
...
//                            .playOn(findViewById(R.id.edit_area));
    ...
    public void addRandomMine() throws InitializationException {

        if(mMineFlagsRemainingCount == Math.pow(mBoard.getDimension(),2))
        {
            publishGameResult(false);
        }
        else if (++mMineFlagsRemainingCount <= Math.pow(mBoard.getDimension(),2))
        {
            mBoard.addRandomMine();
            mGameManager.publishFlagsRemainingCount(mMineFlagsRemainingCount);
        }
    }
